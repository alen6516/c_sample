...
{
    HashTable *table;
    u32 total_size = sizeof(HashTable) + sizeof(Node *) * size;
...
table_insert(HashTable *table, Node *node)
{
    ...
    Node *head = table->array[key];

    node->next = head;
    table->array[key] = node;
...
table_remove(HashTable *table, Node *node)
{
    ...
    Node *curr;
    curr = table->array[key];

    if (!curr) return NULL;

    Node **next = &table->array[key];
