...
once found, memmove to overrite the duplicated one. Performance is bad.
...
int main() {
    ...
    start = clock();
    ret_len = removeDuplicates(arr, size);		// straight and easy way
    end = clock();
    ...
    start = clock();
    ret_len = _removeDuplicates(arr2, size);	// memove way
    end = clock();
    for (int i=0; i < ret_len; i++) {
        printf("%d, ", arr[i]);
    }
