...
static void *spin_trylock_add(void *arg)
{
    while ((count) < *(int*)arg) {
...
void test_spin_trylock_add(int thr_num, int add_num)
{
    ...
    for (int i=0; i < thr_num; i++) {
        pthread_create(&thr_arr[i], NULL, spin_trylock_add, (void*)&add_num);
        pthread_join(thr_arr[i], NULL);
    }
    end_t = clock();
    total_t = (double)(end_t - start_t) / CLOCKS_PER_SEC;

    printf("test_spin_trylock_add: =======================\n");
    printf("a = %lu\n", a);
